...
class Ruckusing_MigratorUtil
{
     ...
     * 
     * @param string  $directory   The directory of tasks
    ...
    public function getRunnableMigrations($directory, $direction, 
        ...
        foreach ($runnable as $migration) {
            ...
            if ($direction == 'up' 
                && in_array($migration['version'], $executed)
            ) {
                continue;
            }
    ...
    {
        return $this->_executedMigrations();
    }	
    ...
    public static function getMigrationFiles($directory, $direction)
    {
        ...
        if (!is_dir($directory)) {
            die(
                "\nRuckusing_MigratorUtil - ({$directory}) "
                . "is not a directory.\n"
            );
        }
    ...
    /**
     * Find the index of the migration in the set of migrations 
     * that match the given version
     * 
     * @param array  $migrations The table of migrations
     * @param string $version    The version searched
     *
     * @return integer
     */
    private function _findVersionIndex($migrations, $version)
    {
        //edge case
        if (is_null($version)) {
            return null;
    ...
    /**
     * Query the database and return a list of migration versions 
     * that *have* been executed
     *
     * @return array
	 */
    private function _executedMigrations()
    {
