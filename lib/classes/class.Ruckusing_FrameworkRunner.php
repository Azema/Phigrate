...
class Ruckusing_FrameworkRunner
{
    ...
    public function execute()
    {
        ...
        if ($this->_taskMgr->hasTask($this->_curTaskName)) {
            ...
            throw new Ruckusing_InvalidTaskException($msg);
        }
        $this->_logger->debug(__METHOD__ . ' End');
        return $output;
    }
    ...
    private function _verifyDbConfig()
    {
        ...
        if (! is_array($this->_dbConfig) 
            ...
            $this->_logger->err($msg);
            throw new Ruckusing_MissingConfigDbException('Error: ' . $msg);
        }
        ...
        if (! array_key_exists('type', $this->_activeDbConfig)) {
            ...
            $this->_logger->err($msg);
            $exception = new Ruckusing_MissingAdapterTypeException('Error: ' . $msg);
        }
        if (! array_key_exists('host', $this->_activeDbConfig)) {
            $msg = '"host" is not set for "' . $env . '" DB';
            $this->_logger->err($msg);
            $exception = new Ruckusing_MissingConfigDbException('Error: ' . $msg);
        }
    ...
    private function _getAdapterClass($dbType)
    {
        ...
        case 'pgsql':
        default:
            throw new Ruckusing_InvalidAdapterTypeException(
